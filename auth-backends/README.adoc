...
```
rudder.auth.oauth2.provider.registrations=keycloak,okta
```
...
xref:plugins:multi-tenants.adoc[Multi-tenants plugin] documentation explains more details about tenants configuration.
...
See an example configuration file below for details about these property values.
...
See an example configuration file below for details about that property.

=== Example configuration for `keycloak` provider

In this section, we use `keycloak` as OIDC provider, with the name `keycloak` configured to identify that provider in the Rudder properties configuration file.

Keycloak is an open source identity provider with extensive documentation, tooling support with Docker, and reproducibility features.
This can be useful since OAUTHv2/OpenID Connect configuration can be a bit complicated and full of jargon.

In the remaining part of this section, you may want to change `keycloak` by the name you chose to identify your OIDC provider in Rudder.

You can copy the following example into `/opt/rudder/etc/rudder-web.properties.d/30-oidc-keycloak-authentication.properties`.
...
# your company uses both "Keycloak" and "Okta", you can choose "keycloak" and
# "okta" (how original) identifiers:
rudder.auth.oauth2.provider.registrations=keycloak,okta

# Now, configure Keycloak related properties. You will need to do
# the same for each provider with an identifier.

# The identity service provider name as it will be displayed in Rudder
rudder.auth.oauth2.provider.keycloak.name=Keycloak
# A more detailed explanation message displayed in authentication page.
rudder.auth.oauth2.provider.keycloak.ui.infoMessage=OpenID Connect SSO (Keycloak)

# In Oauth2/OIDC, a client (ie, Rudder) is identifier by a pair of credentials:
# - 1/ an id,
# - 2/ a corresponding secret key.
#
# 1/ Identifier of the client you created in your IdP for Rudder.
#    In Keycloak, it will be managed under a given realm from: https://my-keycloak.com/admin/console/#/<realm>/clients/.
#
rudder.auth.oauth2.provider.keycloak.client.id=my-client-id
#
# 2/ The corresponding "client secret" generated by Keycloak.
#    It could be show from within the client "Credentials" tab.
rudder.auth.oauth2.provider.keycloak.client.secret=PCdHxhMKXkVo5D353O9L1ffkI8KlUYdu
#
...
rudder.auth.oauth2.provider.keycloak.scope=openid email profile
#
...
rudder.auth.oauth2.provider.keycloak.userNameAttributeName=email
#
...
rudder.auth.oauth2.provider.keycloak.uri.auth=https://my-keycloak.com/realms/<realm>/protocol/openid-connect/auth
rudder.auth.oauth2.provider.keycloak.uri.token=https://my-keycloak.com/realms/<realm>/protocol/openid-connect/token
rudder.auth.oauth2.provider.keycloak.uri.userInfo=https://my-keycloak.com/realms/<realm>/protocol/openid-connect/userinfo
rudder.auth.oauth2.provider.keycloak.uri.jwkSet=https://my-keycloak.com/realms/<realm>/protocol/openid-connect/certs
#
...
rudder.auth.oauth2.provider.keycloak.client.redirect=https://my-external-rudder-hostname/rudder/login/oauth2/code/{registrationId}
#
#
...
rudder.auth.oauth2.provider.keycloak.grantType=authorization_code
# Authentication type - Rudder only supports client_secret_basic and client_secret_post.
rudder.auth.oauth2.provider.keycloak.authMethod=client_secret_basic
...
rudder.auth.oauth2.provider.keycloak.roles.enabled=true
...
rudder.auth.oauth2.provider.keycloak.roles.attribute=rudderroles
...
rudder.auth.oauth2.provider.keycloak.roles.override=true
...
rudder.auth.oauth2.provider.keycloak.roles.mapping.entitlements.rudder_admin=administrator
rudder.auth.oauth2.provider.keycloak.roles.mapping.entitlements.rudder_readonly=readonly
# By default, there are restrictions to roles that The IdP can assign, to match only the role mapped in above entitlements.
# When the following property is true, roles that don't appear in `mapping.entitlements` will be filtered-out (default).
# When it is false, any role from the IdP is mapped to a Rudder role if the same role name exists.
rudder.auth.oauth2.provider.keycloak.roles.mapping.restricted=true
...
#rudder.auth.oauth2.provider.keycloak.roles.mapping.reverseEntitlements.readonly2=rudder_readonly
# And here, we map the IdP role "name=Alice,ou=users" to Rudder role "readonly"
#rudder.auth.oauth2.provider.keycloak.roles.mapping.reverseEntitlements.readonly=name=Alice,ou=users
...
rudder.auth.oauth2.provider.keycloak.enableProvisioning=true
```
...
You can copy the following example into `/opt/rudder/etc/rudder-web.properties.d/40-oidc-okta-authentication.properties`.
...
# an identifier for each service as a comma separated list.
# Identifier should be lower case ascii, -, _. For example, if
# your company uses both "Okta" and "Keycloak", you can choose "okta" and
# "keycloak" (how original) identifiers:
rudder.auth.oauth2.provider.registrations=okta,keycloak
...
# When the following property is true, roles that don't appear in `mapping.entitlements` will be filtered-out (default).
# When it is false, any role from the IdP is mapped to a Rudder role if the same role name exists.
rudder.auth.oauth2.provider.okta.roles.mapping.restricted=true
# In some case, you OIDC provided roles will contains illegal character that can't be use in the
# left part of the `entitlement` key - typically an equal. You can reverse the order of mapping with
# the `reverseEntitlements`, in which the Rudder role name is on the left and the OIDC IdP role
# name is on the right. If the same key is defined in both `entitlements` and `reverseEntitlements`,
...
**Invalid issued at time in ID token**

In the log, you see:

----
2026-02-11 14:34:47+0100 INFO  application.authentication - Rudder authentication attempt for principal 'unknown' with backend 'oidc': failure: [invalid_id_token] An error occurred while attempting to decode the Jwt: The ID Token contains invalid claims: {iat=2026-02-11T13:33:40Z}
2026-02-01 14:34:47+0100 WARN  application.authentication - Login authentication failed for user 'unknown' from IP '127.0.0.1|X-Forwarded-For:xx.xxx.xxx.xxx, xx.xxx.xxx.xxx, xx.xxx.xxx.xxx': [invalid_id_token] An error occurred while attempting to decode the Jwt: The ID Token contains invalid claims: {iat=2026-02-11T13:33:40Z}
----

This is likely because clocks between Rudder and the auth server mismatch, the response of the invalid claims contains IAT (issued at) timestamp.
The clocks should be synchronised or allow time offset in your authentication server if possible.
...
rudder.auth.oauth2.jwt.provider.okta.roles.mapping.restricted=true
...
rudder.auth.oauth2.jwt.provider.okta.tenants.mapping.restricted=true
...
# By default, we use the value of the attribute `sub` 
# but some OIDC configuration use something else
# (`client_id`, `cid`... ) so this can be changed with that property.
#
# Optional, default "sub".
# The name of the attribute in the access token that should be used to map to
# Rudder API token ID.
#
rudder.auth.oauth2.opaque.provider.okta.userNameAttributeName=sub
